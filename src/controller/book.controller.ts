...
import { allBooks, createBook, deleteBook, getBook, updateBook } from "../repository/book.repository";
...
export const one = async (request: Request<GetBookInput>, response: Response, next: NextFunction) => {
    try {
...
export const save = async (request: Request<{}, {}, CreateBookInput>, response: Response, next: NextFunction) => {
    try {
...
export const update = async (request: Request<UpdateBookInput['params'], {}, UpdateBookInput['body']>, response: Response, next: NextFunction) => {
    try {
        const id = parseInt(request.params.bookId)
        if (isNaN(id)) {
            throw new AppError({
...
export const remove = async (request: Request<DeleteBookInput>, response: Response, next: NextFunction) => {
    try {
        const id = parseInt(request.params.bookId)
        if (isNaN(id)) {
            throw new AppError({
        ...
        let bookToRemove = await deleteBook(id)
